<?php
    ...
    $req = $pdo->prepare("SELECT name, path FROM `image` WHERE id=?");
    $req->execute([$post["id_image"]]);
    $image = $req->fetch();
            ...
            if ($_FILES["image"]["size"] < 2000000) {
                ...
                if (in_array(strtolower($fileExtension), ["png", "jpg", "jpeg", "webp", "gif"])) {
                    if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
                        ...
                        $req = $pdo->prepare("UPDATE `post` SET content=:content, id_image=:id_image WHERE id=:id");
...
<div class="container mt-3">
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <textarea name="content" id="content" class="form-control" placeholder="Le contenu de votre tweet"><?= $post["content"] ?></textarea>

        <label for="image" class="form-label">Image</label>
        <input type="file" name="image" id="image" class="form-control">
        
        <?php if(!$_POST) { ?>
            <img class="mt-3" src="<?= $image["path"] ?>" alt="<?= $image["name"] ?>" height="220px"><br>
        <?php } ?>
